...
function bindStyleHelper(options) {
  ...
  var attrKeys = Ember.keys(attrs).filter(function(item, index, self) {
    return (item.indexOf("unit") == -1) && (item !== "static") && (item !== "boundOptions");
  });
  ...
  attrKeys.forEach(function(attr) {
    ...
    Ember.assert(fmt("You must provide an expression as the value of bound attribute. You specified: %@=%@", [attr, property]), typeof property === 'string');
...
export default bindStyleHelper;